...
class Login extends CI_Controller
{
	...
	public function login_validation()
	{
		...
		if($this->form_validation->run())
		{
			...
			$data=$this->Admin_Model->get_all_data($name);

			$epassword=$this->encryption->decrypt($id->user_password);

			if($password==$epassword)
			{
				if($id->user_role_id==1)
				{
				...
				else
				{
					...
					$data = array('name' => $name, 'college_id' =>24, 'loggedin'=> TRUE, 'user'=>'coadmin');
